...
Each rucksack has two large compartments. All items of a given type are meant to go into exactly one of the two compartments. The Elf that did the packing failed 
...
for items in input_values:
    if items != "":
        first_container = slice(0, len(items)//2)
        second_container = slice(len(items)//2, len(items))
        
        for letter in list(items[first_container]):
            if letter in items[second_container]:
                if letter not in alphabet:
                    rucksack_sum += upper_alphabet.index(letter)+27
                    break
                else :
                    rucksack_sum += alphabet.index(letter)+1
                break
