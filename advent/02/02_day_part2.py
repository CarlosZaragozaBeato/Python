...
for game in inputValues:
    ...
    if (response == "Y"):
        response = is_tie(opponent)
    elif (response == "X"):
        response = to_lose(opponent)
    elif (response == "Z"):
        response = to_win(opponent)

    if(isTie(opponent) == response):
        # Draw
        totalWinnings += get_points(response) + 3
    else:
        if(toWin(opponent) == response):
            # Best response to opponent
            totalWinnings += get_points(response) + 6
        else:
            # Worst response to opponent
            totalWinnings += get_points(response)
