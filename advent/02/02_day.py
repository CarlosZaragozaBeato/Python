...
for game in input_values:
    values_formatted = game.split(" ")
    
    opponent = values_formatted[0]
    response = values_formatted[1]
    
    if is_tie(opponent) == response:
        total_winnings += get_points(response)+3
    else :
        if to_win(opponent) == response:
            total_winnings += get_points(response)+6
        else: 
            total_winnings += get_points(response)
